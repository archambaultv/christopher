...
-- taxTable info =
--   let incomes = [unitToAmount (x * 1000) | x <- [10..80]] -- Up to 80K
--               ++ [unitToAmount (80000 + x * 5000) | x <- [1..24]] -- Up to 200K
--               ++ [unitToAmount (200000 + x * 10000) | x <- [1..20]] -- Up to 400K
--       incomes2 = map head
--                $ group
--                $ sort
--                $ incomes ++ taxBrackets info
--       info' = sortTaxBrackets info
--   in map (\x -> reportToTableRow (computePersonnalTax info' $ PersonnalTaxInput (salary x) mempty))
--      incomes2
